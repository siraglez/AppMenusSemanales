...
// MARK: - Aviso etiquetado con el miembro al que afecta
struct MemberWarning: Identifiable {
    let memberName: String
    let warning: PreferenceWarning
    
    // id estable a partir del nombre + el mensaje
    var id: String { "\(memberName)-\(warning.id)" }
}
...
func preferenceWarnings(for recipe: Recipe, preferences: UserPreferences?) -> [PreferenceWarning] {
    ...
    let allergenMatches = prefs.allergies.filter { allergen in
        ...
        if let related = intoleranceIngredients[key] {
            return ingredientNames.contains { name in related.contains { name.contains($0) } }
        }
        return ingredientNames.contains { $0.contains(key) }
    }
    if !allergenMatches.isEmpty { result.append(.allergen(allergenMatches)) }
    ...
    if !intoleranceMatches.isEmpty { result.append(.intolerance(intoleranceMatches)) }
    
    // 3. PREFERENCIAS (no me gusta)
    let dislikeMatches = prefs.dislikes.filter { dislike in
        ingredientNames.contains { $0.contains(dislike.lowercased()) }
    }
    if !dislikeMatches.isEmpty { result.append(.disliked(dislikeMatches)) }
    
    return result
}

// MARK: - Función familiar: avisos de varios comensales, con su nombre
func familyPreferenceWarnings(for recipe: Recipe, eaters: [(name: String, prefs: UserPreferences)]) -> [MemberWarning] {
    var result: [MemberWarning] = []
    for eater in eaters {
        let warnings = preferenceWarnings(for: recipe, preferences: eater.prefs)
        for w in warnings {
            result.append(MemberWarning(memberName: eater.name, warning: w))
        }
    }
    return result
}
